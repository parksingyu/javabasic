...
public class Exam05 {
	...
	public static void main(String[] args) {
		...
		하시오.
		a. int[] arr[];
		b. int[] arr = {1,2,3,};  // new 타입[]을 생략해서 바로 값 입력 가능.
		c. int[] arr = new int[5];  // 타입[] 배열이름 new int[길이]
		d. int[] arr = new int[5]{1,2,3,4,5};  // {} 안에 값의 갯수로 배열의 길이가 정해진다 - 두번째 대괄호[] 에 숫자 넣으면 안됨.
		e. int arr[5];  // 배열을 선언할 때는 배열의 크기를 지정할 수 없음.
		f. int[] arr[] = new int[3][];  // new int[][] 괄호 안에 아예 안들어 있으면 문제가 있지만 하나라도 들어 있으면 괜찮.
		 일단패스!!!!
 		int[] arr0[];
		int[] arr1 = {1, 2, 3};
		int[] arr2 = new int[5];
		...
		**
		...
		1,0 > 0,2   1,1 > 1,2   1,2 > 2,2   1,2 > 3,2   1,4 > 4,2
		...
		*/
		...
		result[i][score[i].length] += result[i][j]; // [i][3]을 채워줌
		result[score.length][j] += result[i][j]; //  [5][i]를 채워줌 
		result[score.length][score[i].length] += result[i][j]; //[5][3]을 채워줌
		 */
