	...
	public static void main(String[] args) {
		...
		/*
		  [6-2] 문제6-1에서 정의한 SutdaCard클래스에 두 개의 생성자와 info()를 추가해서 
			...
			정답 : return Math.sqrt((x-x1) * (x-x1) + (y-y1) * (y-y1));
			[실행결과]
			1.4142135623730951
			...
			int hp = 60;	// 현재 체력
			static int weapon	= 6;	// 공격력  //정답1
			static int armor	= 0;	// 방어력  //정답2
			static void weaponUp() {            //정답3
			weapon++;
			}
			static void armorUp() {             //정답4
			armor++;
			}
			}
			...
			[6-10] 다음 중 생성 자에 대한 설명으로 옳지 않은 것은? (모두 고르시오)
			...
			e. 생성자는 오버로딩 할 수 없다.
			정답: b, e
			b - > 생성자는 객체를 초기화 하기 위한것.
			e - > 오버로딩 가능함. 여러개의 생성자 정의 가능 (파라미터로 구별)
			
			
			[6-11] 다음 중 this에 대한 설명으로 맞지 않은 것은? (모두 고르시오)
			a. 객체 자신을 가리키는 참조변수이다.
			b. 클래스 내에서라면 어디서든 사용할 수 있다.  -> 생성자 안에서 사용
			...
			b. long add(long a, long b) { return a+b;} -> 타입이 다르니 가능
			c. int add(byte a, byte b) { return a+b;}  -> 타입이 다르니 가능
			d. int add(long a, int b) { return (int)(a+b);}  -> 타입이 다르니 가능
			...
			e.클래스변수보다 인스턴스변수가 먼저 초기화된다.  -> 클래스변수가 먼저 초기화 된다.
			
			[6-15] 다음중 인스턴스변수의 초기화 순서가 올바른 것은?
			a. 기본값-명시적초기화-초기화블럭-생성자  <-- 배움. ClassMaker2에 필기함.
			b. 기본값-명시적초기화-생성자-초기화블럭
			c. 기본값-초기화블럭-명시적초기화-생성자
			d. 기본값-초기화블럭-생성자-명시적초기화
			
			[6-16] 다음 중 지역변수에 대한 설명으로 옳지 않은 것은? (모두 고르시오)
			a. 자동 초기화되므로 별도의 초기화가 필요없다.             -> 초기화 필요함.
			b. 지역변수가 선언된 메서드가 종료되면 지역변수도 함께 소멸된다.
			c. 매서드의 매개변수로 선언된 변수도 지역변수이다.
			d. 클래스변수나 인스턴스변수보다 메모리 부담이 적다.
			e. 힙(heap)영역에 생성되며 가비지 컬렉터에 의해 소멸된다.   -> call stack에 생성된다.
			...
			b. println메서드를 제외한 나머지 메서드들은 모두 종료된 상태이다. -> 종료된 상태는 아니다.
			c. method2메서드를 호출한 것은 main메서드이다.        
			d. println메서드가 종료되면 method1메서드가 수행을 재개한다.  -> 맞음
			...
			int iv2 = cv;
			static int cv2 = iv;  // 라인 A  // 스태틱 변수 초기화에 인스턴스 변수 사용 불가
			 
			static void staticMethod1() {
			System.out.println(cv);
			System.out.println(iv);	// 라인 B   //스태틱 메소드니까 스태틱  변수만 불러올수 있음.(인스턴스 변수 사용불가)
			}
			...
			staticMethod1();
			instanceMethod1();	// 라인 D  //스태틱 메소드에서는 스태틱 메소드만 불러올 수 있다 (인스턴스 메소드 사용불가)
			}
			}
			...
			정답: ABC123
			     After change: ABC123 < --- 이유는 차차 알아가자 잘 모르겠다.
			...
			완성한 후에 실행해서 주어진 실행결과 와 일치하는지 확인하라.
